...
uint8_t VL53L7CX_RdByte(VL53L7CX_Platform *p, uint16_t reg, uint8_t *val) {
    return st_rd_byte((uint8_t)(p->address >> 1), reg, val);
}

uint8_t VL53L7CX_WrByte(VL53L7CX_Platform *p, uint16_t reg, uint8_t val) {
    return st_wr_byte((uint8_t)(p->address >> 1), reg, val);
}

uint8_t VL53L7CX_WrMulti(VL53L7CX_Platform *p, uint16_t reg,
                          uint8_t *vals, uint32_t size) {
    return st_wr_multi((uint8_t)(p->address >> 1), reg, vals, size);
}

uint8_t VL53L7CX_RdMulti(VL53L7CX_Platform *p, uint16_t reg,
                          uint8_t *vals, uint32_t size) {
...
uint8_t VL53L7CX_Reset_Sensor(VL53L7CX_Platform *p) {
    (void)p;
    /* LPn(CS 핀, GPIO10) — sensor_manager에서 LOW/HIGH로 관리 */
    return 0;
}

void VL53L7CX_SwapBuffer(uint8_t *buf, uint16_t size) {
